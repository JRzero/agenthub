...
export function ProfileSettingsPanel() {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]"><section className="panel p-6"><h2 className="text-xl font-semibold">个人资料</h2><AvatarEditor avatar={avatar} fallback={username.slice(0, 1)} onUpload={(file) => void upload(file)} onRemove={() => void removeAvatar()} /><form onSubmit={save} className="mt-7 space-y-5"><TextField label="用户名" value={username} onChange={setUsername} required /><TextField label="显示名称" value={fullName} onChange={setFullName} /><label className="block"><span className="font-medium">个人描述</span><textarea rows={4} value={description} onChange={(event) => setDescription(event.target.value)} className="mt-2 w-full resize-none rounded-md border border-border bg-surface px-4 py-2.5" /></label><label className="block"><span className="font-medium">邮箱</span><input readOnly value={profile?.email || ""} className="mt-2 w-full rounded-md border border-border bg-subtle px-4 py-2.5 text-text-muted" /><span className="mt-1 block text-xs text-text-muted">邮箱暂不支持修改</span></label>{message && <p className="rounded-md border border-primary/20 bg-primary-soft px-4 py-3 text-sm text-text-muted">{message}</p>}<div className="flex justify-end"><button type="submit" className="button-primary" disabled={saving}>{saving ? "保存中…" : "保存资料"}</button></div></form></section><PasswordPanel apiKey={session?.apiKey || ""} demo={demo} /></div>;
}

function TextField({ label, value, onChange, required }: { label: string; value: string; onChange: (value: string) => void; required?: boolean }) {
  return <label className="block"><span className="font-medium">{label}</span><input value={value} onChange={(event) => onChange(event.target.value)} required={required} minLength={required ? 3 : undefined} className="mt-2 w-full rounded-md border border-border bg-surface px-4 py-2.5" /></label>;
}
