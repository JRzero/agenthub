...
export function Workbench() {
  ...
  const focusAgent = agents.find((agent) => resolveAgentLifecycle(agent).state !== "published" && agent.config?.metadata?.avatar)
    || agents.find((agent) => agent.config?.metadata?.avatar)
    || agents.find((agent) => resolveAgentLifecycle(agent).state !== "published")
    || agents[0];
    ...
    <div className="mx-auto w-full max-w-[1760px] space-y-7 pb-8">
      ...
      <section aria-labelledby="continue-heading">
        ...
        {focusAgent ? <div data-testid="workbench-agent-hero" className="panel grid min-h-[420px] overflow-hidden xl:h-[460px] xl:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.55fr)]">
          ...
          <div className="flex flex-col justify-between border-t border-border p-5 sm:p-6 xl:border-l xl:border-t-0">
            <div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-text-muted">当前创作进度</p><div className="mt-5 flex items-center justify-between text-sm"><span>配置完成度</span><strong className="text-primary">{readiness(focusAgent)}%</strong></div><span className="mt-3 block h-1.5 overflow-hidden rounded-full bg-subtle"><span className="block h-full rounded-full bg-primary" style={{ width: `${readiness(focusAgent)}%` }} /></span><dl className="mt-6 space-y-4 border-t border-border pt-5 text-sm"><div><dt className="text-xs text-text-muted">Agent 编码</dt><dd className="mt-1 truncate font-medium" title={focusAgent.code}>{focusAgent.code || `agent-${focusAgent.id}`}</dd></div><div><dt className="text-xs text-text-muted">运行模型</dt><dd className="mt-1 truncate font-medium">{focusAgent.llm_model_name || focusAgent.model || "尚未配置"}</dd></div></dl></div>
            <div className="mt-7 flex flex-wrap gap-2"><Link href={`/assets/${focusAgent.id}/build`} className="button-primary flex-1">打开工作区<ArrowRight size={16} /></Link><Link href={`/assets/${focusAgent.id}/test`} className="button-secondary">测试</Link></div>
          </div>
