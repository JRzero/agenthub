...
export function MomentSchedulePanel({ agentId }: { agentId: number }) {
  ...
  return <section className="rounded-xl border border-border p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">自动发朋友圈</h3><p className="mt-1 text-xs text-text-muted">由 AI 根据角色人设生成一周排期。</p></div><div className="flex gap-2"><button type="button" onClick={() => void generate()} disabled={busy} className="button-secondary"><CalendarDots size={17} />AI 一键排期</button>{data?.config && <button type="button" onClick={() => void remove()} disabled={busy} className="button-secondary text-danger">关闭自动排期</button>}</div></div>{data?.config ? <div className="mt-4 rounded-lg bg-subtle p-4"><p className="text-sm">每周 {data.config.weekdays.join("、")} · {data.config.daily_times.join("、")} · {data.config.timezone}</p>{data.reasoning && <p className="mt-2 text-xs text-text-muted">{data.reasoning}</p>}<div className="mt-3 flex flex-wrap gap-2">{data.schedules.map((item) => <span key={item.id} className="status-badge bg-surface text-text-muted">{new Date(item.scheduled_at).toLocaleString("zh-CN")} · {item.status}</span>)}</div></div> : <p className="mt-4 rounded-lg border border-dashed border-border p-5 text-sm text-text-muted">尚未启用自动排期。</p>}{message && <p className="mt-3 text-sm text-text-muted">{message}</p>}</section>;
}
