...
export function MomentsPanel({ agentId, agentName }: { agentId: number; agentName: string }) {
  ...
  return <div className="space-y-6"><div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700 dark:border-blue-400/20 dark:bg-blue-400/10 dark:text-blue-200">朋友圈属于运营内容，不进入 Agent 版本。</div><MomentComposer agentId={agentId} agentName={agentName} onPublished={(moment) => setMoments([moment, ...moments])} /><MomentSchedulePanel agentId={agentId} /><section><h3 className="mb-3 text-lg font-semibold">已发布朋友圈</h3><div className="space-y-4">{moments.map((moment) => <article key={moment.id} className="rounded-xl border border-border bg-surface p-5"><div className="flex items-start justify-between gap-3"><div><strong>{moment.agent_name}</strong><p className="mt-1 text-xs text-text-muted">{new Date(moment.created_at).toLocaleString("zh-CN")}</p></div><button type="button" onClick={() => void remove(moment.id)} disabled={busy === `delete-${moment.id}`} className="icon-button hover:bg-danger/5 hover:text-danger" aria-label={`删除朋友圈 ${moment.id}`}><Trash size={18} /></button></div><p className="mt-4 whitespace-pre-wrap leading-7">{moment.content}</p>{moment.thumbnail_urls.length > 0 && <div className="mt-4 grid grid-cols-3 gap-2">{moment.thumbnail_urls.map((url) => <div key={url} className="relative aspect-square overflow-hidden rounded-lg"><Image src={url} alt="朋友圈图片" fill unoptimized sizes="180px" className="object-cover" /></div>)}</div>}<div className="mt-4 text-xs text-text-muted">点赞 {moment.like_count || 0}</div>{moment.comments?.map((item) => <div key={item.id} className="mt-3 rounded-lg bg-subtle px-3 py-2 text-sm"><strong>{item.creator_name}</strong>：{item.content}</div>)}<div className="mt-3 flex gap-2"><input value={commentInputs[moment.id] || ""} onChange={(event) => setCommentInputs((current) => ({ ...current, [moment.id]: event.target.value }))} placeholder="添加创作者评论" className="control-field min-w-0 flex-1" /><button type="button" onClick={() => void comment(moment.id)} disabled={!commentInputs[moment.id]?.trim() || busy === `comment-${moment.id}`} className="button-secondary"><ChatCircle size={17} />发送</button></div></article>)}{!moments.length && <div className="rounded-xl border border-dashed border-border p-10 text-center text-text-muted">暂无朋友圈</div>}</div></section>{message && <p className="text-sm text-text-muted">{message}</p>}</div>;
}
