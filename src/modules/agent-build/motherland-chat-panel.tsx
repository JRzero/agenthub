        ...
        <div className="border-t border-border p-4 sm:p-5">
          ...
          {mode === "chat" ? (
            ...
            <div className="mt-4 rounded-lg border border-border bg-canvas/35 p-4">
              ...
              <label className="mt-4 block"><span className="text-sm font-medium">讨论主题</span><div className="mt-2 flex flex-col gap-2 sm:flex-row"><input value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="输入主题，或让 AI 推荐一个主题" className="h-10 min-w-0 flex-1 rounded-md border border-border px-3 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15" /><button type="button" onClick={() => void generateTopic()} disabled={Boolean(busy)} className="button-secondary shrink-0 whitespace-nowrap"><MagicWand size={16} />{busy === "topic" ? "生成中…" : topic ? "换个主题" : "AI 推荐主题"}</button></div></label>
