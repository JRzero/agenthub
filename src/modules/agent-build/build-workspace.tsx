...
export function BuildWorkspace() {
  ...
  return <div className="-mx-4 -mt-5 sm:-mx-6 lg:-mx-7"><div className="flex flex-wrap items-center gap-3 border-b border-border bg-surface px-4 py-3 sm:px-6 lg:px-7"><div className="mr-auto min-w-0"><div className="flex items-center gap-3"><h1 className="text-lg font-semibold">构建 Agent</h1><SourceBadge source={editor.demo ? "demo" : "live"} /></div><div className="mt-1 flex items-center gap-2 text-xs text-text-muted" aria-live="polite">{editor.dirty ? <><span className="h-2 w-2 rounded-full bg-warning" />有未保存更改</> : <><CheckCircle size={14} className="text-success" />{editor.savedAt ? `已保存于 ${editor.savedAt.toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}` : "已同步最新版本"}</>}{editor.demo && <span>· 仅保存在当前演示会话</span>}</div></div>{editor.saveError && <p className="w-full text-xs text-danger sm:w-auto">{editor.saveError}</p>}<button type="button" onClick={editor.reset} disabled={!editor.dirty || editor.saving} className="button-secondary"><ArrowCounterClockwise size={16} />重置</button><button type="button" onClick={() => void editor.save("draft")} disabled={!editor.dirty || editor.saving} className="button-secondary"><FloppyDisk size={16} />{editor.saving ? "保存中…" : "保存草稿"}</button><button type="button" onClick={() => void saveAndTest()} disabled={editor.saving} className="button-primary"><Play size={16} />保存并测试</button></div><div className="grid min-h-[720px] grid-cols-1 border-b border-border lg:grid-cols-[190px_minmax(0,1fr)] xl:grid-cols-[190px_minmax(420px,1fr)_minmax(340px,0.9fr)]"><BuildSectionRail active={section} onChange={(next) => { setSection(next); setTab("basics"); }} /><BuildEditorPanel section={section} tab={tab} agent={editor.data} draft={editor.draft} errors={editor.validationErrors} knowledgeBases={editor.knowledgeBases} knowledgeLoading={editor.knowledgeLoading} onTabChange={setTab} onPatch={editor.patchDraft} onAgentUpdated={editor.applyAgentUpdate} /><BuildPreview agent={editor.data} draft={editor.draft} /></div></div>;
}
