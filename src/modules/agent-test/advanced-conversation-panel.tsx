...
export function AdvancedConversationPanel({ agent, scenario, messages, widgets, sending, error, demo, memoryBusy, memoryNotice, onSend, onClear, onClearMemory }: {
  ...
  return <section className="flex min-h-[680px] min-w-0 flex-col bg-surface">
    ...
    <div className="flex items-center gap-3 border-b border-border px-5 py-3"><AgentAvatar agent={agent} size={38} className="rounded-full" /><div className="min-w-0"><strong className="block truncate text-sm">{agent.name}</strong><span className="text-xs text-text-muted">测试用户 · {demo ? "演示回答" : "实时模拟"} · {widgets.length} 个输入 Widget</span></div></div>
    ...
    {(error || memoryNotice) && <p className={`border-t px-5 py-2 text-sm ${error ? "border-danger/20 bg-red-50 text-danger" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>{error || memoryNotice}</p>}
    <RuntimeInputBar widgets={widgets} disabled={sending} placeholder="输入测试消息，或添加图片/文档" onSubmit={onSend} />
  </section>;
}
