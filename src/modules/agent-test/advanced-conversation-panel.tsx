...
export function AdvancedConversationPanel({ agent, scenario, messages, widgets, sending, error, demo, memoryBusy, memoryNotice, onSend, onClear, onClearMemory }: {
  ...
  return <section className="flex min-h-[680px] min-w-0 flex-col bg-surface xl:h-full xl:min-h-0 xl:overflow-hidden">
    ...
    <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain bg-canvas/40 p-5" aria-live="polite">
    ...
    {(error || memoryNotice) && <p className={`shrink-0 border-t px-5 py-2 text-sm ${error ? "border-danger/20 bg-red-50 text-danger" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>{error || memoryNotice}</p>}
