...
import { canNarrowPermissions, classifyWorldError, hasPermissionChanges, limitedChangeWorldCode, PREFLIGHT_LABELS, replaceInvitationDecisionDraft, WORLD_READINESS, worldQueryKeys } from "./state";
...
describe("Living World C adapter", () => {
  ...
  it("G4 preserves semantic kinds, empty residents and dated recap revision history", async () => {
    ...
    await expect(worldApi.projection(ctx, "monster")).resolves.toMatchObject({ public_residents: [], timeline: [{ semantic_kind: "rumor" }] });
  ...
  it("deduplicates repeated server preflight categories for stable rendering", async () => {
    ...
    await expect(worldApi.detail(ctx, "monster")).resolves.toMatchObject({ preflight: { missing: ["initial_event_participant_invalid", "structured_rules_required"] } });
  });
...
describe("Living World state contract", () => {
  ...
  it("maps stable conflict, preflight, permission and offline recovery", () => {
    ...
    expect(classifyWorldError(new ApiError("missing", 422, "WORLD_PREFLIGHT_FAILED", { missing: ["title_required"] }))).toMatchObject({ kind: "preflight", missing: ["title_required"] });
