...
export function WorldErrorNotice({ error, onRetry, conflictCopy }: { error: unknown; onRetry?: () => void; conflictCopy?: () => void }) {
  ...
  return <div role={state.kind === "conflict" || state.kind === "not-found" ? "alert" : "status"} className="w-full min-w-0 rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm dark:border-rose-400/20 dark:bg-rose-400/10"><div className="flex min-w-0 gap-2"><WarningCircle className="mt-0.5 shrink-0 text-danger" size={18} /><div className="min-w-0 flex-1"><strong>{state.kind === "not-found" ? "无法访问" : "操作未完成"}</strong><p className="mt-1 break-words text-text-muted">{state.message}</p>{state.missing.length > 0 && <ul className="mt-2 list-disc pl-5">{state.missing.map((item) => <li className="break-words" key={item}>{item}</li>)}</ul>}<div className="mt-3 grid grid-cols-1 gap-2 sm:flex sm:flex-wrap">{onRetry && <button type="button" className="button-secondary w-full sm:w-auto" onClick={onRetry}>刷新真源</button>}{state.kind === "conflict" && conflictCopy && <button type="button" className="button-secondary w-full sm:w-auto" onClick={conflictCopy}>复制我的修改</button>}</div></div></div></div>;
}
