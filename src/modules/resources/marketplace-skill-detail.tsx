...
export function MarketplaceSkillDetail({ skill, ownedSkill, demo, busy, onAdd, onAttach, onManage }: {
  ...
  return <aside className="p-5">
    ...
    <dl className="mt-5 grid grid-cols-2 gap-4 border-y border-border py-4 text-sm"><div><dt className="text-xs text-text-muted">阶段</dt><dd className={`status-badge mt-1 ring-1 ring-inset ${stage.className}`}>{stage.label}</dd></div><div><dt className="text-xs text-text-muted">实现</dt><dd className="mt-1 font-medium">{skill.implementation_type}</dd></div></dl>
