...
export function SkillsLibraryNext() {
  ...
  return <div>
    <div className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-4 sm:px-6"><label className="relative w-full max-w-sm"><MagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" size={18} /><input value={library.search} onChange={(event) => library.setSearch(event.target.value)} placeholder="搜索技能名称或能力" className="h-10 w-full rounded-md border border-border bg-surface pl-10 pr-3 outline-none focus:border-primary" /></label><select value={library.category} onChange={(event) => library.setCategory(event.target.value)} className="h-10 rounded-md border border-border bg-surface px-3 text-sm">{library.categories.map((item) => <option key={item}>{item}</option>)}</select><button type="button" onClick={() => library.selected && library.setAttachOpen(true)} disabled={!library.selected} className="button-primary ml-auto"><Plus size={17} />添加到 Agent</button></div>
    ...
    <div className="grid min-h-[600px] min-[1400px]:grid-cols-[150px_minmax(0,1fr)_310px]"><aside className="border-b border-border p-4 min-[1400px]:border-b-0 min-[1400px]:border-r"><div className="flex gap-2 overflow-x-auto min-[1400px]:block min-[1400px]:space-y-1">{library.categories.map((item) => <button key={item} type="button" onClick={() => library.setCategory(item)} className={`whitespace-nowrap rounded-md px-3 py-2 text-left text-sm ${library.category === item ? "bg-primary-soft font-medium text-primary" : "text-text-muted hover:bg-subtle"}`}>{item}</button>)}</div></aside><SkillCatalogue skills={library.filtered} selectedId={library.selected?.id} creatorSkills={library.creatorSkills} onSelect={library.setSelectedId} /><MarketplaceSkillDetail skill={library.selected} ownedSkill={library.ownedSkill} demo={library.demo} busy={library.busy} onAdd={() => void library.addToWorkspace()} onAttach={() => library.setAttachOpen(true)} onManage={library.setManageSkill} /></div>
