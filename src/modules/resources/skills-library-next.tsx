  ...
  return <div>
    <div className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-4 sm:px-6">
      <label className="relative w-full max-w-sm">
        ...
        <input value={library.search} onChange={(event) => library.setSearch(event.target.value)} placeholder="搜索技能名称或能力" className="control-field w-full pl-10" />
      </label>
      <Select
        ariaLabel="技能分类"
    ...
    {library.error && <div className="mx-4 mt-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-danger dark:border-rose-400/20 dark:bg-rose-400/10 dark:text-rose-200 sm:mx-6">{library.error}</div>}
