...
export function SkillsLibraryNext() {
  ...
  return <div>
    <div className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-4 sm:px-6"><label className="relative w-full max-w-sm"><MagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" size={18} /><input value={library.search} onChange={(event) => library.setSearch(event.target.value)} placeholder="搜索技能名称或能力" className="h-10 w-full rounded-md border border-border bg-surface pl-10 pr-3 outline-none focus:border-primary" /></label><select value={library.category} onChange={(event) => library.setCategory(event.target.value)} className="h-10 rounded-md border border-border bg-surface px-3 text-sm">{library.categories.map((item) => <option key={item} value={item}>{getSkillCategoryLabel(item)}</option>)}</select><button type="button" onClick={() => library.selected && library.setAttachOpen(true)} disabled={!library.selected} className="button-primary ml-auto"><Plus size={17} />添加到 Agent</button></div>
