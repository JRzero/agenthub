...
export function RevenueWorkspace() {
  ...
  return <main className="space-y-5 p-6 lg:p-8"><header className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><h1 className="text-2xl font-semibold">收益中心</h1><SourceBadge source={source} /></div><button type="button" className="button-primary"><FileText size={18} />查看演示结算单</button></header>
    <select className="min-h-10 rounded-md border border-border bg-surface px-4" value={month} onChange={(event) => setMonth(event.target.value)} aria-label="结算月份"><option value="2026-07">2026年7月</option><option value="2026-06">2026年6月</option></select>
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,2fr)_430px]"><article className="panel p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">收入与成本趋势</h2><div className="flex gap-2"><select className="rounded-md border border-border bg-surface px-3 py-2 text-sm" value={granularity} onChange={(event) => setGranularity(event.target.value)} aria-label="统计粒度"><option value="day">按日</option><option value="week">按周</option></select><select className="rounded-md border border-border bg-surface px-3 py-2 text-sm" value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value)} aria-label="收益来源"><option value="all">全部来源</option><option value="runtime">运行调用</option><option value="license">Agent 授权</option></select></div></div><div className="mt-4 flex gap-5 text-sm"><span className="flex items-center gap-2"><i className="size-2 rounded-full bg-primary" />收入（积分）</span><span className="flex items-center gap-2"><i className="size-2 rounded-full bg-success" />成本（积分）</span></div><TrendChart labels={REVENUE_LABELS} series={chartSeries} ariaLabel={`${month}收入与成本趋势，${granularity === "day" ? "按日" : "按周"}展示`} /></article>
