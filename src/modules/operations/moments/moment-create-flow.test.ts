...
import { getDefaultMomentAgentId } from "./moment-create-flow";

describe("Moment Agent selection", () => {
  const agents = [
  ...
  it("keeps an available initial Agent", () => {
    expect(getDefaultMomentAgentId(agents, 3)).toBe(3);
  });

  it("falls back to the first published Agent", () => {
    expect(getDefaultMomentAgentId(agents, 1)).toBe(2);
  });

  it("does not select an unpublished Agent when none are available", () => {
    expect(
      getDefaultMomentAgentId(
        [
          { id: 1, current_version_id: null },
