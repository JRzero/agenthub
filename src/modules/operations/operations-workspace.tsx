  ...
  return (
    <div className="-mx-4 -mt-6 bg-surface sm:-mx-6 lg:-mx-7">
      ...
      {tab === "sessions" ? loading ? <div className="py-24 text-center text-text-muted">正在加载共享会话…</div> : <div>
        ...
        <div className={`grid min-h-[720px] ${viewMode === "by-agent" ? "lg:grid-cols-[520px_minmax(0,1fr)] min-[1500px]:grid-cols-[520px_minmax(0,1fr)_300px]" : "lg:grid-cols-[320px_minmax(0,1fr)] min-[1400px]:grid-cols-[320px_minmax(0,1fr)_300px]"}`}>
...
function AgentSessionNavigator({ agents, users, sessions, selectedAgentId, selectedUserId, selectedSessionId, loadingUsers, loadingSessions, onAgent, onUser, onSession }: { agents: Agent[]; users: SharedUser[]; sessions: SharedSessionRow[]; selectedAgentId: number | null; selectedUserId: number | null; selectedSessionId?: number; loadingUsers: boolean; loadingSessions: boolean; onAgent: (value: number) => void; onUser: (value: number) => void; onSession: (row: SharedSessionRow) => void }) {
  return <aside className="grid min-h-0 border-b border-border bg-surface lg:grid-cols-[170px_170px_minmax(0,1fr)] lg:border-b-0 lg:border-r">
...
function Column({ title, children }: { title: string; children: ReactNode }) {
  return <div className="flex min-h-0 flex-col border-b border-border lg:border-b-0 lg:border-r last:border-r-0"><div className="border-b border-border px-3 py-2 text-xs font-semibold text-text-muted">{title}</div><div className="min-h-0 flex-1 overflow-y-auto p-2">{children}</div></div>;
}
