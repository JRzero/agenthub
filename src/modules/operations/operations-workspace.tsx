...
export function OperationsWorkspace() {
  ...
  }, [demo, reloadRevision, session?.apiKey, tab, workspaceCode]);
  ...
  return (
    <div className="-mx-4 -mt-6 bg-canvas sm:-mx-6 lg:-mx-7">
      <header className="border-b border-border bg-canvas px-4 pt-6 sm:px-6 lg:px-7">
        ...
        <p className="mt-1.5 text-sm text-text-secondary">查看应用中的真实共享会话与内容运营状态</p>
      ...
      {tab === "sessions" && error && <div className="error-feedback mx-4 mt-3 flex items-center justify-between gap-3 sm:mx-6"><span>{error}</span><button type="button" className="button-secondary control-compact shrink-0" onClick={() => { setError(""); setReloadRevision((value) => value + 1); }}><ArrowClockwise size={14} />重试</button></div>}
      {tab === "sessions" ? loading ? <div className="py-24 text-center text-text-muted">正在加载共享会话…</div> : <div>
        <div className="flex flex-wrap items-center gap-2 border-b border-border bg-canvas px-4 py-3 sm:px-6 lg:px-7"><span className="text-xs text-text-muted">视图</span><button type="button" onClick={() => setViewMode("by-agent")} className={`rounded-lg border px-3 py-1.5 text-xs font-medium ${viewMode === "by-agent" ? "border-primary bg-primary-soft text-primary" : "border-border text-text-muted hover:bg-subtle"}`}>按 Agent 查看</button><button type="button" onClick={() => setViewMode("all")} className={`rounded-lg border px-3 py-1.5 text-xs font-medium ${viewMode === "all" ? "border-primary bg-primary-soft text-primary" : "border-border text-text-muted hover:bg-subtle"}`}>全部共享会话</button></div>
        <div className="overflow-x-auto">
          <div className={`grid h-[calc(100vh-244px)] min-h-[640px] min-w-[900px] ${viewMode === "by-agent" ? "grid-cols-[420px_minmax(360px,1fr)_minmax(280px,300px)]" : "grid-cols-[260px_minmax(360px,1fr)_minmax(280px,300px)]"}`}>
