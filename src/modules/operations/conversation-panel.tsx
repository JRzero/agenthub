  ...
  return (
    <section className="flex min-h-0 flex-col bg-surface">
      <header className="border-b border-border px-4 py-3">
        <div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold">{sessionLabel(row)}</h2><p className="mt-1 text-xs text-text-muted">会话 #{row.session.id} · {humanLabel(row)} · {row.agent.name}</p></div><button type="button" onClick={onVerify} className={`min-h-8 rounded-md px-3 text-xs font-medium ${row.session.verified ? "bg-emerald-50 text-emerald-700" : "border border-border text-text-muted hover:bg-subtle"}`}>{row.session.verified ? <span className="flex items-center gap-1"><CheckCircle size={15} weight="fill" />已认证</span> : "标记已认证"}</button></div>
