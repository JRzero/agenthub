...
export function ConversationPanel({ row, messages, loading, sending, onVerify, onComment }: { row: SharedSessionRow | null; messages: SessionMessage[]; loading: boolean; sending: boolean; onVerify: () => void; onComment: (content: string) => void }) {
  ...
  return (
    <section className="flex min-h-0 flex-col bg-surface">
      ...
      <footer className="border-t border-border p-3"><label className="flex items-center gap-2 rounded-lg border border-border bg-surface-elevated px-3"><input value={comment} onChange={(e) => setComment(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && comment.trim() && !sending) { onComment(comment); setComment(""); } }} placeholder="输入创作者评论，将推送给用户" className="h-10 min-w-0 flex-1 bg-transparent text-sm outline-none" /><button type="button" disabled={!comment.trim() || sending} onClick={() => { onComment(comment); setComment(""); }} className="rounded-md bg-primary p-2 text-canvas disabled:opacity-40" aria-label="发送创作者评论"><PaperPlaneRight size={17} /></button></label></footer>
    </section>
  );
}
