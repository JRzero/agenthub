...
function SkillsStep({ state, dispatch, skills, loading }: { state: CreateAgentState; dispatch: React.Dispatch<Parameters<typeof createAgentReducer>[1]>; skills: CreatorSkill[]; loading: boolean }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex items-center justify-between gap-3"><label className="relative min-w-0 flex-1"><span className="sr-only">搜索技能</span><MagnifyingGlass size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索技能名称或能力" className="control-field w-full pl-9" /></label><span className="whitespace-nowrap text-sm text-text-muted">已选择 {state.selectedSkillIds.length} 个</span></div>
