...
- 接入 Client 跟随状态和不绑定 Client 的平台当前版本通用 ZIP 导出入口，不提供 Client 独立选版本、更新或回退。
...
- 接入 `GET/PUT /agents/{id}`、`POST /agents/{id}/publish`、版本历史/详情/创建草稿接口，以及 Agent Client 查询、运行版本、Agent 级导出记录创建和 ZIP Blob 下载接口。
