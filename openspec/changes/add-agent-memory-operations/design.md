...
UI 优化遵循现有 AgentHub tokens 与 4/8 像素节奏，不修改全局主题：关系通道使用靛蓝紫，情绪通道使用青蓝，暂未获取继续使用琥珀。四项摘要拆为可响应重排的独立 KPI 卡片；覆盖、分布与诊断采用明确的标题、数值、单位和文字标签，确保颜色不是唯一信息。刷新按钮保持单一主操作并提供至少 44px 的触达高度；桌面采用主内容与诊断侧栏，窄屏回落为单列且不产生横向滚动。

字体继续继承 AgentHub 全局 `font-sans`，不为单页引入新的字族。页面标题与其他 Agent Asset 二级页面对齐为 `text-xl`，常规模块标题保持 `text-base/text-lg`。关系与情绪通道采用紧凑密度：容器使用 `p-4/p-5`，完整度百分比收为 `text-lg` 的行内指标带，稀疏样本不会因为 100% 数值形成展示型大卡或额外留白。
