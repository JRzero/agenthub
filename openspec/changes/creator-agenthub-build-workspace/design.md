...
The section rail is the only persistent editor navigation. Persona contains the system prompt and example conversations, Runtime contains provider settings plus reasoning/tool-call display controls, and Safety contains visibility and unavailable policy boundaries. The ordered rail places Runtime directly after Persona, then Skills before Knowledge, so the primary behavior and capability workflow precedes supporting context. This preserves one draft and save payload without presenting an unrelated second navigation axis on every section.
