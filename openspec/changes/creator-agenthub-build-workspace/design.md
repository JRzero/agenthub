...
### 8. Keep configuration ownership in one section hierarchy

The section rail is the only persistent editor navigation. Persona contains the system prompt and example conversations, Runtime contains provider settings plus reasoning/tool-call display controls, and Safety contains visibility and unavailable policy boundaries. The ordered rail places Runtime directly after Persona, then Skills before Knowledge, so the primary behavior and capability workflow precedes supporting context. The Overview composition follows the same core priority after combining Identity and Persona into one summary row: Runtime follows identity/persona, and Knowledge follows Skills. This preserves one draft and save payload without presenting an unrelated second navigation axis on every section.

Alternative: keep global Basic, Examples, and Advanced tabs above every section. Rejected because the tabs conflict with the selected section, expose low-frequency controls in unrelated contexts, and turn a temporary migration grouping into permanent information architecture.
