...
The workspace SHALL provide a landing page that uses current Agent data to select a focused Agent, resume work, summarize real lifecycle state, show deterministic pending tasks, open recent assets, and present Agent selection as a persistent layered carousel with silent automatic advance.
...
#### Scenario: Silent automatic advance

- **WHEN** more than one Agent is available and the carousel remains eligible for 3000ms
- **THEN** the Workbench SHALL request the next Agent exactly once without showing a pause/resume control
- **AND** SHALL wait for the current 720ms transition to complete before scheduling another full 3000ms interval

#### Scenario: Automatic advance pause conditions

- **WHEN** the carousel is hovered, contains keyboard focus, the document is hidden, a transition is active, reduced motion is preferred, or only one Agent exists
- **THEN** automatic advance SHALL NOT run
- **AND** a full 3000ms interval SHALL begin after an eligible transient pause ends
...
- **AND** a new full 3000ms interval SHALL begin only after the manual transition is idle and all pause conditions are clear
...
- **AND** a single Agent SHALL NOT start autoplay or show an autoplay control
